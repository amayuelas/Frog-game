...
{
	Iterador posicio = m_cua.getInici();
	int aleatori =  rand() % (CONSTANT_AFEGIR_VEHICLE/m_nivell + 1);
	m_vehicle.mouAIniciCarril(m_iniciCarrilX, m_iniciCarrilY);
	bool solapa = m_vehicle.getAreaOcupada().solapa(m_cua.getUltim().getAreaOcupada());
	...
	if (!esDinsCarril(m_cua.getPrimer().getAreaOcupada()))
	{
		m_cua.treu();
		if (m_cua.esBuida())
			m_cua.afegeix(m_vehicle);
		...
		{
			if ((aleatori >= CONSTANT_AFEGIR_VEHICLE/m_nivell) && (!solapa))
				m_cua.afegeix(m_vehicle);
		}
	}
